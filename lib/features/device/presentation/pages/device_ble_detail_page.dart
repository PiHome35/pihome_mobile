...
class DeviceBleDetailPage extends StatelessWidget {
  final BleDeviceEntity device;

  const DeviceBleDetailPage({
    super.key,
  ...
  Widget _buildConnectedState(BuildContext context, BleDetailConnected state) {
    return Column(
      children: [
        ...
        FilledButton(
          onPressed: () {
            // context.go(AppRoutes.deviceSetup);
            AppRoutes.navigateToDeviceSetup(context, widget.device);
          },
          child: Text('Setup'),
        ),
      ],
